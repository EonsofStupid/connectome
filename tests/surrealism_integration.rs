...
mod surrealism_integration {
	...
	///
	/// Uses `CARGO_BIN_EXE_connectome` when present (set by `cargo test` / cargo-nextest) so the path
	/// stays valid under nextest's per-test directory layout. Falls back to deriving
	/// `target/debug/connectome` from `std::env::current_exe()` for runners that omit it.
	fn surreal_bin() -> PathBuf {
		if let Some(p) = std::env::var_os("CARGO_BIN_EXE_connectome")
			.or_else(|| std::env::var_os("NEXTEST_BIN_EXE_connectome"))
		{
